...
and all of them have respective strong and weak points. Different applications may require absolute invisibility of the secret information, 
while others require a large secret message to be hidden. 
...
If a person or persons views the object that the information is hidden inside of he or she will have no idea that there is any hidden
information  therefore the person will not attempt to decrypt the information.

What steganography essentially does is exploit human perception, human senses are not trained to look for files that have information inside
of them, although this software is available that can do what is called Steganography. 
...
This LSB IMAGE STEGANOGRAPHY project is completed writenn in c language.
In this project we can hidden .txt file message in bmp file and generate stego.bmp file and that stego.bmp file is use to decode secret.txt message.
In order run the code compile by using gcc *.c(to compile all .c file at once) or gcc (followed by all .c <filename>).
After compile (a.out) file wil be generated,then executed by,

For Encoding (-e)
(./a.out -e  secret.txt beautiful.bmp) or (./a.out -e  secret.txt beautiful.bmp stego.bmp)

For Decoding (-d)
(./a.out -d  stego.bmp) or (./a.out -d  stego.bmp decoded_secret.txt)
